...
- モバイル表示 — 検索しやすい「さがす」タブと、一日の流れを確認できる「まとめ」タブを切り替え可能
- モバイルで活動、役割、複数の空き時間帯、キーワードによる絞り込みが可能
- モバイルのタブと絞り込み欄は、スクロール中も固定表示
- モバイルのまとめ表示で、募集中の役割別空き枠を色付きチップで表示
...
├── index.html       # フロントエンド — デスクトップグリッド、モバイルカード、モーダル申込み・キャンセルフォーム
...
- `index.html`のクライアント側ユーティリティと状態管理ロジック
  イベント索引の構築、レイアウト判定、モバイル表示モード、メッセージ表示、クライアント側の正規化など
