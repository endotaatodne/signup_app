...
- 申込みまたはキャンセル：`ClientModal.html`がブラウザ側の入力を事前検証して公開Apps Scriptサービスを呼び出します。`SignupService.gs`が信頼できるサーバー側で再検証し、イベントポリシーとレート制限を確認し、`LockService`でシート更新を直列化して安全な結果を返した後、クライアントがグリッドを再取得します。
